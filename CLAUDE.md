...
Cargo workspace (resolver 3, edition 2024) with eight crates:
...
- **`datamancer-winsec`** — shared Windows security-primitives crate: token/handle identity (process token user SID, pipe owner SID) and process integrity-level reading, plus the pure cross-platform integrity classifier that carries the CI coverage. The workspace's single audited `unsafe` surface for these primitives (`forbid(unsafe_code)` off Windows, `deny` + one scoped `ffi` allow on Windows). `datamancer-client` and `datamancerd` depend on it (Windows only); `datamancer-core` and `datamancer-credentials` never do.
...
Workspace-wide lints: `clippy::pedantic = deny` (with `priority = -1` so individual lints can be relaxed per call site). Member crates opt in via `[lints] workspace = true`. **`#![forbid(unsafe_code)]` in all eight crates**, with EXT-1 exceptions where Win32/FFI is unavoidable: the iceoryx2 transport (its gate confirms `ZeroCopySend` is a safe derive), `datamancer-winsec` (`#![deny(unsafe_code)]` on Windows with one scoped `#[allow(unsafe_code)]` confined to its `ffi` module — the workspace's single audited surface for token/handle identity and integrity reads), and — on **Windows only** — `datamancerd`, which relaxes to `#![deny(unsafe_code)]` with a *single* scoped `#[allow(unsafe_code)]` confined to its `win_control` module for named-pipe creation + SDDL (#36). `datamancer-client` is `#![forbid(unsafe_code)]` on every platform (its `win_pipe` delegates all FFI to `datamancer-winsec`); non-Windows builds of every crate stay `forbid`; every `unsafe` block carries a `// SAFETY:` proof. See those crates' docs.
