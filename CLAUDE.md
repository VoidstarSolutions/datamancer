...
- **`datamancer-core`** — pure types and trait surface (`Provider`, `LiveHandle`, `TapLog`, `HistoricalCache`, `ReplaySource`, `EventSink`) plus the event model (`MarketEvent`, `Trade`, `Bar`, `Quote`, `Control`), `Instrument`, `Price`, and `health.rs`'s `HealthView` (a versioned, per-symbol-only reduction of `SystemSnapshot` for app-facing health rendering). No I/O, minimal deps. Provider/storage/transport implementation crates depend only on this — never on the orchestrator.
