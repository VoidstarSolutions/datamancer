...
| 2 | **Health-push over WS** — `WsRequest::WatchHealth { id }` (after `ws.rs:48`) + `id()` arm (`:56-61`); a dedicated pushed `Health` frame carrying `datamancer_core::HealthView`, intercepted in `WsClient::run_reader` **before** `from_wire` (keeps "one frame = one `MarketEvent`" intact) → a separate health channel; daemon `dispatch` arm (`ws/conn.rs:316`) spawns a push task (mirrors `spawn_pump:325`) emitting the stamped view on `self.diag_interval`, reusing `server.rs:stamped_health_view:53` | `protocol/ws.rs`, `transport-ws/wire.rs`, `datamancerd/ws/conn.rs`, `client/ws.rs` |
...
4. **Pushed-health wire shape** — **LOCKED: a dedicated pushed `Health` frame on a separate `WsClient` channel** (2026-07-19). `WatchHealth` is a request/reply *subscribe* (follows #40's template), but the ongoing `HealthView` pushes are server-initiated with no client `id` to echo, so they cannot use the reply-by-`id` oneshot demux. They ride a distinct frame the client's `run_reader` intercepts **before** `from_wire` (preserving the "one frame = one `MarketEvent`" invariant) and routes to a dedicated health channel that surfaces as `HealthStream`. Rejected: overloading `WsReply.health` with a reserved sticky `id` (fights the pending-oneshot demux) and routing health through `to_wire`/`from_wire` (it is not a `MarketEvent`).
