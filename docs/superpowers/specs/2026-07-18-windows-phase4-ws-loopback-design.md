...
5. **CI support-boundary reversal** — **BLESSED by Zach** (2026-07-19, Slack). `ci.yml:84-87` documented (per the open-sourcing spec) that Windows gets only the ws-portable subset, not the daemon/`app`. Zach's blessing: *"removing the limitations preventing the daemon running on Windows, then yes. Once it's possible to run on windows we want the daemon to run on windows in CI as well."* So worklist item 6 is **unblocked, and desired** — once the daemon runs on Windows (WS-loopback), CI should run it there too (this feeds Phase 5's full-Windows-CI goal). When item 6 lands, rewrite the `ci.yml:84-87` boundary comment to record the lifted scope + this decision.
